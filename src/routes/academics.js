...
router.put('/classes/:id', requireRole('headteacher', 'admin'), async (req, res) => {
  try {
    const existing = await prisma.academicClass.findFirst({ where: { id: req.params.id, schoolId: req.schoolId } });
    if (!existing) return res.status(404).json({ error: 'Not found' });
    const data = {};
    if (typeof req.body.name === 'string' && req.body.name.trim()) data.name = req.body.name.trim();
    if (typeof req.body.section === 'string' && data.name !== undefined) data.section = req.body.section;
    if (typeof req.body.teacher === 'string') data.teacher = req.body.teacher.trim() || 'Unassigned';
    const cls = await prisma.academicClass.update({ where: { id: req.params.id }, data });
    await logAudit(req, 'update', 'class', cls.id, { name: cls.name, teacher: cls.teacher });
    res.json(cls);
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
...
router.put('/subjects/:id', requireRole('headteacher', 'admin'), async (req, res) => {
  try {
    const existing = await prisma.subject.findFirst({ where: { id: req.params.id, schoolId: req.schoolId } });
    if (!existing) return res.status(404).json({ error: 'Not found' });
    const data = {};
    if (typeof req.body.name === 'string' && req.body.name.trim()) data.name = req.body.name.trim();
    if (typeof req.body.code === 'string' && data.name !== undefined) data.code = req.body.code.trim().toUpperCase();
    if (typeof req.body.teacher === 'string') data.teacher = req.body.teacher.trim() || 'Unassigned';
    const subject = await prisma.subject.update({ where: { id: req.params.id }, data });
    await logAudit(req, 'update', 'subject', subject.id, { name: subject.name, teacher: subject.teacher });
    res.json(subject);
  } catch (err) {
    res.status(400).json({ error: err.message });
  }

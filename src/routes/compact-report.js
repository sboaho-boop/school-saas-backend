...
const { getCompactReport } = require('../lib/hubtel-compact-report');
...
router.post('/check', authenticate, requireRole('headteacher', 'admin', 'accountant'), async (req, res) => {
  try {
    ...
    if (!apiId || !apiKey) {
      return res.status(503).json({ error: 'Compact report API not configured. Set CREDIT_SCORE_API_ID and CREDIT_SCORE_API_KEY.' });
    }

    const result = await getCompactReport({
      ghanaCard,
      identifierName,
      identifierNationality,
      identifierDob,
    ...
    console.error('Compact report error:', err);

...
const HUBTEL_SEND_TO_BANK_STATUS_URL = 'https://smrsc.hubtel.com/api/merchants';
...
function checkSendToBankStatus(disbursementAccount, clientReference, authHeader) {
  return new Promise((resolve, reject) => {
    const url = `${HUBTEL_SEND_TO_BANK_STATUS_URL}/${disbursementAccount}/transactions/status?clientReference=${encodeURIComponent(clientReference)}`;
    const parsed = new URL(url);
    const options = {
      hostname: parsed.hostname,
      path: parsed.pathname + parsed.search,
...
router.get('/send-to-bank/status/:clientReference', authenticate, requireRole('headteacher', 'admin', 'accountant'), async (req, res) => {
  try {
    const { clientReference } = req.params;
    if (!clientReference) return res.status(400).json({ error: 'clientReference required' });
    ...
    const clientId = school.hubtelClientId || process.env.HUBTEL_CLIENT_ID;
    const clientSecret = school.hubtelClientSecret || process.env.HUBTEL_CLIENT_SECRET;
    const disbursementAccount = school.hubtelDisbursementAccount || process.env.HUBTEL_DISBURSEMENT_ACCOUNT;

    if (!clientId || !clientSecret || !disbursementAccount) {
      return res.status(400).json({ error: 'Hubtel disbursement credentials not configured' });
    }

    const auth = Buffer.from(`${clientId}:${clientSecret}`).toString('base64');
    const authHeader = `Basic ${auth}`;

    const result = await checkSendToBankStatus(disbursementAccount, clientReference, authHeader);
    res.json(result);
  } catch (err) {
    console.error('Send-To-Bank status check error:', err);
    res.status(500).json({ error: err.message });
  }
});

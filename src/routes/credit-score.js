...
const { checkCreditScore } = require('../lib/hubtel-credit-score');
...
router.post('/check', authenticate, requireRole('headteacher', 'admin', 'accountant'), async (req, res) => {
  try {
    ...
    if (!apiId || !apiKey) {
      return res.status(503).json({ error: 'Credit score API not configured. Set CREDIT_SCORE_API_ID and CREDIT_SCORE_API_KEY.' });
    }
